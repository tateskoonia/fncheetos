...
class Aimbot:
    ...
    config_file_path = os.path.join('lib', 'config.json')
    ...
    def __init__(self, box_constant = 416, collect_data = False, mouse_delay = 0.0001, debug = False):
        ...
        if torch.cuda.is_available():
            ...
            print(colored("cuda is malfunctioning", "red"))
    ...
    def start(self):
        ...
        while True:
            ...
            cv2.putText(frame, f"FPS: {int(1/(time.perf_counter() - start_time))}", (5, 30), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255), 2)
            cv2.imshow("Discord", frame)
